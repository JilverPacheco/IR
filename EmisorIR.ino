...
Serial.print(" \n");  
 ...
 escribepin4(inicio);
  ...
  escribemsj4();
 Serial.print(" \n");
checksum= t + h + agua + tierra;
invchecksum = ~checksum;
 Serial.print(" \n");
  Serial.print(" invchecksum ");
  Serial.print(invchecksum);
  mensaje1= invchecksum;
 Serial.print(" \n");
 escribepin1(inicio);
  delayMicroseconds(pausa);
  escribepin1(cero);
...
void escribemsj3(){/////////////////////// MENSAJE
 ...
 for (int nbit = 31; nbit >= 0; nbit--) {
  ...
  void escribemsj1(){/////////////////////// MENSAJE
for (int nbit = 31; nbit >= 0; nbit--) {
         Serial.print(bitRead(mensaje1, nbit));
    if (bitRead(mensaje1, nbit)) {
      escribepin1(uno);
    }  else {
      escribepin1(cero);
      delayMicroseconds(cero);
